...
class TestGBIFRecordShow:
    ...
    def test_action_through_ckan(self, requests_mock):
        """
        Just to make sure the plugin is all wired up right run the success test through
        the get_action toolkit function instead of calling the gbif_record_show function
        directly.
        """
        gbif_id = 'test'
        mock_response = MagicMock(status_code=200)
